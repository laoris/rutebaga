...
public class Vector2D implements GenericVector2D<Double, Vector2D>
{
	...
	public boolean equals(Object obj)
	{
		...
		if (Double.doubleToLongBits(comp[0]) != Double
				.doubleToLongBits(other.comp[0]))
			return false;
		if (Double.doubleToLongBits(comp[1]) != Double
				.doubleToLongBits(other.comp[1]))
			return false;
		return true;
	}
	...
	{
		return this.getX() * other.getX().doubleValue() + this.getY()
				* other.getY().doubleValue();
	}

	public static MutableVector2D getProjection(GenericVector2D vector,
			GenericVector2D axis)
	{
		double dot = vector.dot(axis);
		double mag = axis.getMagnitude();
		MutableVector2D result = new MutableVector2D(axis);
		result.multiplyBy(dot / (mag * mag));
		return result;
	}
}
