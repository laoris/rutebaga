 ...
 */
public class AvatarInitializationActionInterpreter implements
		UserActionInterpreter, TextFieldListener
{
	...
	public void installActionInterpreter(final GameDaemon daemon,
			final Game game, final UserInterfaceFacade facade)
	{
		Command accept = new Command()
		{
			...
			public void execute()
			{
				...
				UserActionInterpreter gpai = new GamePlayActionInterpreter(gi
						.getWorld(), avatars[job], gi.getDisplayedStats());

				avatars[job].setName(name);

				facade.clearWarningBox();
				// Connascence: you must deactivate this interpreter before
