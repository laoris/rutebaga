 ...
 * receive user input from the View, and feed Commands to the View to be
 * displayed to the user. Interpreters are components of the GameDaemon. When
 * the daemon activates a UserActionInterpreter, it calls
 * installActionInterpreter on the interpreter. The uninstallActionIterpreter
 * operation is invoked when an interpreter is deactivated.
 * 
 ...
 * notifying this one.
 * 
 * All interpreters may also accept KeyEvents from the View, which will push
...
public interface UserActionInterpreter extends KeyListener {
	 ...
	 * on the GameDaemon stack. That is, eventsFallThrough determines whether,
	 * when this interpreter receives an event (such as a tick or key press), it
	 * should be the last interpreter to be notified of the event (i.e., the
	 * GameDaemon should not pass it down the stack).
	 * 
	 ...
	 * The GameDaemon typically invokes this operation when activating an
	 * interpreter in the controller. Calling installActionInterpreter is a
