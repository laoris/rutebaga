 ...
 * default feasibility Rule returns true if and only if the set of Commands
 * is non-empty and all composited Commands' isFeasible operations return true.
 * 
...
public class CompositeCommand implements Command {
	...
	/**
	 * A private shared copy of the default feasibility Rule,
	 * which only returns true for isFeasible if this
	 * CompositeCommand's set of Commands is non-empty and
	 * all its Commands return true for isFeasible.
	 */
	 ...
	 * Equivalently, adds the specified Command from this CompositeCommand,
	 * if it has not previously been added.
	...
	/**
	 * Determines whether it is feasible to execute this CompositeCommand,
	 * using this CompositeCommand's Rule for determining feasibility.
	 * 
	 ...
	 * CompositeCommand while it is executing, but changes will not affect
	 * an execution already in progress.
	 * 
	 * A Command is only executed if its isFeasible method returns true. 
	 * 
	 * (Consider this example: execute() is running on a CompositeCommand.
	 * One of the executed Commands removes another Command from this
	 * CompositeCommand.  The removed command will still be executed by
	 * this invocation of execute().  However, a second invocation of
	 * execute() on this CompositeCommand which occurs after the Command
	 * is removed but before the first execution of this CompositeCommand's
	 * execute() completes will not execute the removed Command.)
	 */
	public void execute() {
		// Iterate over an array so it's OK to add/remove commands while executing
	...
	private static class DefaultFeasibilityRule implements Rule<Iterator<Command>> {
		/**
		 * The default feasibility Rule returns true if and only if the
		 * set of Commands is non-empty and all composited Commands'
