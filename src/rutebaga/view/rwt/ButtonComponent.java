...
public class ButtonComponent extends ViewComponent
{
	...
	private FontAttribute font;// XXX: why isn't this at the ViewComponent
								// level?
	 ...
	 * 
	 * @param label
	 *            The string written over this ButtonComponent.
	 */
	...
	{
		/*
		 * if (toggle) drawToggled(draw); else drawUntoggled(draw);
		 */
		if (toggle)
	...
	/*
	 * private void drawUntoggled(Drawer draw) { draw.setAttribute(untoggled);
	 * draw.drawRectangle(getLocation(), width, height);
	 * 
	 * Point centeredText = new Point(); FontMetrics fm = draw.getFontMetrics();
	 * 
	 * centeredText.x = this.getLocation().x + (width / 2) -
	 * (fm.stringWidth(label) / 2); centeredText.y = this.getLocation().y +
	 * (height / 2) + (font.getFont().getSize() / 2);
	 * 
	 * draw.drawString(centeredText, label); }
	 * 
	 * private void drawToggled(Drawer draw) { draw.setAttribute(toggled);
	 * draw.drawRectangle(getLocation(), width, height);
	 * 
	 * Point centeredText = new Point(); FontMetrics fm = draw.getFontMetrics();
	 * 
