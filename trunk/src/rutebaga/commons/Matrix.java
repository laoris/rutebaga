...
public class Matrix
{
	...
	{
		int dimY = vectors.length;
		Integer dimX = null;
		for(int idx=0; idx<vectors.length; idx++)
		{
			if(vectors[idx] == null) dimY--;
			else if(dimX == null) dimX = vectors[idx].getDimension();
		}
		this.dimY = dimX;
		this.dimX = dimY;
		components = new double[dimX*dimY];
		for(int idx=0, ptr=0; idx<vectors.length; idx++)
		{
			if(vectors[idx] != null)
			{
				for(int loop=0; loop<dimX; loop++, ptr++)
					components[ptr] = vectors[idx].get(loop);
			}
		}
	}
